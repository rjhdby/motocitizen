...
class DetailsMenuController(val activity: FragmentActivity, val accident: Accident) {
    ...
    fun itemSelected(item: MenuItem): MenuAction {
        ...
        return when (item.itemId) {
            ...
            R.id.action_share                          -> {
                Router.share(activity, accident.getAccidentTextToCopy())
                MenuAction.SHARE
            }
            R.id.action_hide_info, R.id.menu_hide_info -> {
                if (generalLayout.visibility == View.VISIBLE)
                    showGeneralLayout(View.INVISIBLE)
                else
                    showGeneralLayout(View.VISIBLE)
                MenuAction.HIDE_INFO
            }
