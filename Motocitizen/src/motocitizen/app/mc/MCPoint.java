...
public class MCPoint {
	...
	public MCPoint(JSONObject json) throws MCPointException {
		Map<String, String> data = buidDataSet(json);
		if (checkPrerequisites(data) == false)
	...
	private Map<String, String> buidDataSet(JSONObject json) {
	...
	public List<Integer> getSortedMessagesKeys() {
		List<Integer> keys = new ArrayList<Integer>(messages.keySet());
	...
	public List<Integer> getSortedVolunteersKeys() {
		List<Integer> keys = new ArrayList<Integer>(volunteers.keySet());
		Collections.sort(keys);
		return keys;
	}

	public List<Integer> getSortedHistoryKeys() {
		List<Integer> keys = new ArrayList<Integer>(history.keySet());
		Collections.sort(keys);
		return keys;
	}

	public boolean hasInOwners() {
	...
	private void makeMessages(JSONArray json) {
		if (messages == null)
			messages = new HashMap<Integer, MCMessage>();
		Map<Integer, MCMessage> newMessages = new HashMap<Integer, MCMessage>();
	...
	private int countUnreadedMessages() {
		int counter = 0;
	...
	public void resetMessagesUnreadFlag() {
		resetMessagesUnreadFlag(false);
	}

	public void resetMessagesUnreadFlag(boolean status) {
		for (int i : messages.keySet()) {
			messages.get(i).unread = status;
		}
	}

	private void makeVolunteers(JSONArray json) {
		if (volunteers == null)
			volunteers = new HashMap<Integer, MCVolunteer>();
		Map<Integer, MCVolunteer> newVolunteers = new HashMap<Integer, MCVolunteer>();
		for (int i = 0; i < json.length(); i++) {
			try {
	...
	private void makeHistory(JSONArray json) {
		if (history == null)
			history = new HashMap<Integer, MCPointHistory>();
		history.clear();
		for (int i = 0; i < json.length(); i++) {
			try {
	...
	public FrameLayout createAccRow(Context context) {
		...
		TextView unread = (TextView) fl.findViewById(R.id.accident_row_unread);
		...
		if (!med.equals("mc_m_na")) {
			sb.append(", " + getMedText());
		}
		sb.append("(" + getDistanceText() + ")\n" + address + "\n" + descr);
		general.setText(sb);
		// time.setText(Const.timeFormat.format(created.getTime()));
		time.setText(MCUtils.getIntervalFromNowInText(created));
		int unreaded = countUnreadedMessages();
		String msgText = "<b>" + String.valueOf(messages.size()) + "</b>";
		if (unreaded > 0) {
			msgText += "<font color=#C62828><b>(" + String.valueOf(countUnreadedMessages()) + ")</b></font>";
		}
		unread.setText(Html.fromHtml(msgText));
		return fl;
	}
	...
	private View.OnClickListener accRowClick = new View.OnClickListener() {
